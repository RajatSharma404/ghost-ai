...
import { suggestAlternatives, runAlternativesDirect } from "@/trigger/suggest-alternatives"
...
export async function POST(request: Request) {
  ...
  if (direct) {
    try {
      const report = await runAlternativesDirect({
      ...
      console.error("Direct alternatives error:", err)
      return Response.json(
        { error: "Failed to generate architecture alternatives." },
        { status: 500 }
      )
    }
  }
  ...
  try {
    const handle = await tasks.trigger<typeof suggestAlternatives>("suggest-alternatives", {
      projectId: project.id,
      roomId,
      chatHistory,
      nodes,
